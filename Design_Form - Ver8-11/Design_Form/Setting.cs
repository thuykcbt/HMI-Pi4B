...
namespace Design_Form
{
    public partial class Setting : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void simpleButton5_Click(object sender, EventArgs e)
        {
            if(camera==0)
            {
                PLC_Communication.Model_PLC.Button_momentory_2[4] = true;
            }
            if (camera == 1)
            {
                PLC_Communication.Model_PLC.Button_momentory_2[5] = true;
            }
            //Job_Model.Statatic_Model.Dino_lites[camera].SETPARAMETERCAMERA("Contrast", Job_Model.Statatic_Model.model_run.Cameras[camera].Jobs[treejob].Contrast);
           // Job_Model.Statatic_Model.Dino_lites[camera].SETPARAMETERCAMERA("Gain", Job_Model.Statatic_Model.model_run.Cameras[camera].Jobs[treejob].Brightness);
            ...
            Cycletime.Restart();
            InputIMG =Job_Model.Statatic_Model.Dino_lites[camera].Shot();
            update_capture();
            Cycletime.Stop();
            if (camera == 0)
            {
                PLC_Communication.Model_PLC.Button_momentory_2[4] = false;
            }
            if (camera == 1)
            {
                PLC_Communication.Model_PLC.Button_momentory_2[5] = false;
            }
